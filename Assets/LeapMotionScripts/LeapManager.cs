...
{
  public Vector3 handRot=new Vector3(0,0,0);
	...
	// returns the rotation of the currently tracked hand
	public Quaternion GetHandQuat()
	{
	  if((leapHand != null) && leapHand.IsValid)
	    return Quaternion.LookRotation(LeapToUnity(leapHand.Direction,true));
	  else
	    return Quaternion.identity;
	}
		...
		foreach(Pointable finger in leapFrame.Pointables)
		{
			if(finger.IsValid)
			{
				...
				if(finger.Hand != null && finger.Hand.IsValid)
				{
					...
					Leap.Vector handBase = hand.StabilizedPalmPosition + (-hand.Direction * 100);
					//Quaternion handQ = Quaternion.LookRotation(LeapToUnity(-hand.Direction, false));
					//handRot = handQ.eulerAngles;
					handRot = LeapToUnity(hand.Direction*50, false);
